<!-- Manage Notification Language Translate Start -->


<el-drawer custom-class="bpa-drawer__language-translate" :visible.sync="open_manage_notification_translate_language">
			...
			<div v-if="empty_selected_language == 0" v-for="(field_language, field_language_ind) in bookingpress_get_selected_languages">
				<div v-if="bookingpress_current_selected_lang == field_language_ind" v-for="(lang_fields, lang_field_key) in language_fields_data[field_language_ind]" class="bpa-bmw__block">
					<div class="bpa-mw__title">
						<h4 v-html="(typeof bookingpress_notification_manage_language_section_title[lang_field_key] !== 'undefined')?bookingpress_notification_manage_language_section_title[lang_field_key]:''"></h4>
					</div>
						...
						<template>
							<div class="bpa-mw__form">
								<el-form-item v-for="(lang_field_data, lang_field_data_key) in lang_fields">
									...
									<el-input class="bpa-form-control"  v-model="language_data[field_language_ind][lang_field_key][lang_field_data_key]" :type="(lang_field_data.field_type == 'text')?'text':'textarea'" :rows="(lang_field_data.field_type == 'text')?1:8"  :placeholder="lang_field_data.field_label">
									</el-input>								
								</el-form-item>
							</div>
						</template>
	...
	<div class="bpa-dlt__footer">
		<el-button @click="open_manage_notification_translate_language = false;" class="bpa-btn bpa-btn--primary"><?php esc_html_e( 'Okay', 'bookingpress-multilanguage' ); ?></el-button>
	</div> 
</el-drawer>

<!-- Manage Notification Language Translate Over -->
